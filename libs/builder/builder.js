...
Vvveb.Builder = {
	...
	cloneNode: function (node) {
		// Se o nó não for fornecido, usa o elemento selecionado.
		node = Vvveb.Builder.selectedEl;

		// Cria um clone do nó.
		var clone = node.clone(true); // 'true' para clonar também os descendentes do nó.

		// Insere o clone logo após o nó original.
		function generateUniqueId() {
			let id;
			do {
				id = "id" + Math.floor(Math.random() * 10000);
			} while ($(`#${id}`).length > 0); // Verifica se o ID já existe em algum elemento na página
			return id;
		}

		// Verificar se não existe id e dar um id aleatório
		if (!clone.attr("id")) {
			clone.attr("id", generateUniqueId());
		}
		node.after(clone);

		// Clique no clone (isso parece um pouco estranho e pode precisar de revisão dependendo do que você deseja alcançar).
		clone.click();

		//mudar id randomicamente

		// Esta linha parece estar tentando obter o primeiro elemento filho do clone, mas a função 'get' não é padrão do DOM.
		// Se você estiver usando jQuery, isso faria sentido, mas, caso contrário, pode ser desnecessário.
		// var element = clone.get(0); // Potencialmente desnecessário ou incorreto.

		// Adiciona a mutação ao histórico de desfazer/refazer.
		Vvveb.Undo.addMutation({
			type: 'childList',
			target: node.parentNode,
			addedNodes: [clone], // Deveria ser o clone, não um elemento potencialmente indefinido.
...
Vvveb.FileManager = {
	...
	init: function (allowedComponents = {}) {
		...
		$(this.tree).on("click", ".duplicate", function (e) {
			let element = $(e.target).closest("li");
			Vvveb.FileManager.duplicatePage(element, e, true);
			e.preventDefault();
			return false;
		});
	...
	duplicatePage: function (element, e) {
		let page = element[0].dataset;
		let _self = this;

		if (confirm(`Are you sure you want to duplicate "${page.file}" template?`)) {
			let newfile = prompt("Enter new file name for duplicated template", page.file);
			if (newfile) {
				$.ajax({
					type: "POST",
					url: duplicateUrl, //set your server side save script url
					...
					success: function (data) {
						...
						if (data.success) {
							...
							data["file"] = newfile;
							data["title"] = newName;
							//separar nome do arquivo por / para pegar penultima stringfolder

							//verificar se existe / e ese existir pega o valor depois do /
							let parts = newfile.split("/");
							let folder = parts[parts.length - 2]; // Pega o penúltimo elemento


							data["folder"] = folder;
							Vvveb.FileManager.addPage(baseName, data);

							// Carrega a nova página duplicada
							Vvveb.FileManager.loadPage(baseName);
...
Vvveb.Breadcrumb = {
	...
	init: function () {
		...
		$(this.tree).on("click", ".breadcrumb-item", function (e) {
			...
			e.preventDefault();
		}).on("mouseenter, touchstart", ".breadcrumb-item", function (e) {
			let node = $($(this).data("node"));
			node.get(0).dispatchEvent(new MouseEvent("mousemove, touchmove", {
				bubbles: true,
				cancelable: true,
			}))
			//node.css("outline","1px dashed blue");
